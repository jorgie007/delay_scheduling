...
# scheduler_type = "naive_fair"
# scheduler_type = "fair"
# scheduler_type = "delay_fifo"
scheduler_type = "delay_hfs"
...
class Scheduler:
        ...
        for step in range(experiment_duration):
            ...
            if step % 100 == 0 and show_progress:
                print("At step:", step, "\tQueue length:", len(self.queue), "\tActive jobs:", len(self.active_jobs),
                      "\t\tN active nodes", len(self.busy_nodes))
    ...
    def schedule(self):
        ...
        if len(self.queue) != 0:
            ...
            elif scheduler_type == "delay_hfs":
                self.delay_hfs()

            elif scheduler_type == "naive_fair":
                self.naive_fair()
    ...
    def delay_hfs(self):
        W_1 = 15  # [5, 10, 15] = ["High throughput" , "Sticky slots", "Small jobs"]
        if len(self.free_nodes) != 0:
            freeNode = [self.free_nodes[0]]
            ...
            for j in self.queue:
                if j.wait > W_1:
                    ...
                    self.queue.remove(j)
                    self.active_jobs.append(j)
                    j.assign_nodes(freeNode)  # run task on free node
    ...
    def naive_fair(self):
        if len(self.free_nodes) != 0:
            ...
            self.queue.sort(key=ticks) # Sort based on job duration
